...
function _renderComment(d){
  ...
  const bannerItem=c.bannerThemeId?BANNER_THEMES.find(t=>t.id===c.bannerThemeId):null;

  return ''+
  '<div class="yt-comment'+(bannerItem?' has-banner':'')+'" id="yc-'+d.id+'">'+
    (bannerItem?
      '<video class="yt-comment-banner-bg" muted loop playsinline autoplay preload="metadata" src="'+bannerItem.url+'"></video>'+
      '<div class="yt-comment-banner-overlay"></div>'
    :'')+
  ...
  try{
    ...
    try{
      ...
      if(mySnap.exists()){
        ...
        myBanner=md.bannerThemeId||null;
      }
    }catch(e){}
...
async function applyBannerTheme(item,targetId){
  targetId=targetId||'profileBannerVideo';
  ...
  video.onerror=async function(){
    ...
    if(triedBlob){
      ...
      if(targetId==='profileBannerVideo')showToast('⚠️ Video banner gagal dimuat, cek koneksi/link');
      return;
    }
    triedBlob=true;
    try{
...
// Terapin tema banner user yang lagi login ke elemen video manapun (dipake buat background komentar dll)
async function applyMyBannerThemeTo(targetId){
  const el=document.getElementById(targetId);
  if(!el)return;
  let bannerId=null;
  try{bannerId=localStorage.getItem('hidaka_selected_banner');}catch(e){}
  if(!bannerId){el.style.display='none';return;}
  const item=BANNER_THEMES.find(t=>t.id===bannerId);
  if(!item){el.style.display='none';return;}
  await applyBannerTheme(item,targetId);
}
...
async function signInWithGoogle(){
  ...
  const {GoogleAuthProvider}=window._fbAuth;
  const provider=new GoogleAuthProvider();
  provider.setCustomParameters({prompt:'select_account'});

  // Popup gampang nge-hang diem-diem di browser mobile (Firebase gak selalu bisa
  // "denger" hasil dari popup-nya balik), jadi di HP langsung pake redirect biar reliable
  if(isMobile){
    try{
      const {signInWithRedirect}=window._fbAuth;
  ...
  try{
    const {signInWithPopup}=window._fbAuth;
